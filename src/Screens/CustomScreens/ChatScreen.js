...
const ChatScreen = ({route}) => {
  ...
  const sendMessage = () => {
    // console.log(currentUser[0].data);
    Keyboard.dismiss();
    db.collection('chats')
      .doc(route.params)
      .collection('messages')
      .add({
        ...
        message: input,
        user: currentUser[0].data.email,
        image: currentUser[0].data.image
          ? currentUser[0].data.image
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <>
            <ScrollView contentContainerStyle={{paddingTop: 15}}>
              {message &&
              message.map(({ id, data }) => {
                return data.user === currentUser[0].data.email ? (
